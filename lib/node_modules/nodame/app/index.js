...
if (nodame.argv.config !== undefined) {
    if (nodame.argv.config.substring(0,1) !== '/') {
        configStream = path.normalize(sprintf('%s/%s', nodame.appPath(), nodame.argv.config));
    } else {
        configPath = path.normalize(nodame.argv.config);
    }
} else {
    configStream  = path.normalize(sprintf('%s/%s/main.ini', nodame.appPath(), 'configs'));
}
