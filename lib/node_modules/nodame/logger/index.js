var logger = (function () {
    ...
    if (nodame.settings.config.logger.enable) {
        var client              = nodame.settings.config.logger.client || 'syslog';

        switch (client) {
            case 'sentry':
                var raven        = require('raven');
                var sentryClient = new raven.Client(nodame.settings.config.logger.sentry_dns);
                break;
            ...
            default:
                ...
                var accessFile   = path.normalize(nodame.settings.config.logger.syslog.access_stream);
                var accessStream = fs.createWriteStream(accessFile, {flags: 'a'});
                var mailer       = require('nodame/mailer');
                var logFile      = path.normalize(nodame.settings.config.logger.syslog.error_stream);
