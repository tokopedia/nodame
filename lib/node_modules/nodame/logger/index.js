...
var loggerCfg = nodame.config('logger');
var clientCfg = loggerCfg.clients;
...
var logger = (function () {
    ...
    if (loggerCfg.enable) {
        if (clientCfg.datadog.enable) {
            var datadog         = require('nodame/datadog');
            var clientStatsD    = datadog.getClient();
        }

        if (clientCfg.sentry.enable) {
            var raven        = require('raven');
            var sentryClient = new raven.Client(clientCfg.sentry.dns);
        }

        if (clientCfg.syslog.enable) {
            var fs           = require('fs');
            var path         = require('path');
            var accessFile   = path.normalize(clientCfg.syslog.access_stream);
            var accessStream = fs.createWriteStream(accessFile, {flags: 'a'});
            var mailer       = require('nodame/mailer');
            var logFile      = path.normalize(clientCfg.syslog.error_stream);
            var logStream    = fs.createWriteStream(logFile, {flags: 'a'});
            var Log          = require('log');
            var Logger       = new Log('debug', logStream);
        }

        if (clientCfg.morgan.enable) {
    ...
    var error = function () {
        return function (req, res, next) {
            log = (function () {
                ...
                var stat = (function () {
                    if (loggerCfg.enable && clientCfg.datadog.enable) {
    ...
    var access = function () {
        if (loggerCfg.enable && clientCfg.morgan.enable) {
            if (nodame.isDev()) {
